...
<head>
    ...
    <title>Dokumen Memerlukan Revisi - e-QMS</title>
...
<body style="font-family: 'Plus Jakarta Sans', Arial, sans-serif; background-color: #F4F5F6; margin: 0; padding: 30px 15px; color: #1A1A1A;">
    <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #ffffff; border: 1px solid #E5E7EB; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05);">
        ...
        <!-- COLOR ACCENT BAR (RED) -->
        <tr>
            <td height="4" bgcolor="#DC2626"></td>
        </tr>
        ...
        <tr>
            <td style="padding: 36px 28px;">
                ...
                <p style="font-size: 13px; line-height: 1.6; color: #4B5563; margin: 0 0 24px 0;">
                    @if(!empty($isCreator))
                        Dokumen SOP berikut dikembalikan oleh Peninjau untuk dilakukan perbaikan. Status dokumen saat ini dikunci sementara (<code>need_revision</code>) hingga berkas revisi baru Anda unggah.
                    @else
                        Dokumen SOP berikut dikembalikan oleh Peninjau untuk dilakukan perbaikan oleh Pembuat Dokumen. Status dokumen saat ini dikunci sementara (<code>need_revision</code>).
                    @endif
                </p>
                ...
                <table width="100%" cellpadding="12" cellspacing="0" style="border-collapse: collapse; border: 1px solid #E5E7EB; border-radius: 8px; margin-bottom: 20px; overflow: hidden;">
                    ...
                    <tr>
                        <td style="font-size: 12px; font-weight: 600; color: #6B7280; border-bottom: 1px solid #F3F4F6;">Diminta Oleh</td>
                        <td style="font-size: 12px; color: #1A1A1A; border-bottom: 1px solid #F3F4F6; font-weight: 600;">{{ $requester->full_name ?? $requester->username }} <span style="font-size: 11px; color: #6B7280;">({{ $requester->role }})</span></td>
                    </tr>
                    <tr>
                        <td style="font-size: 12px; font-weight: 600; color: #6B7280; border-bottom: 1px solid #F3F4F6;">Versi Dokumen</td>
                        <td style="font-size: 12px; color: #DC2626; border-bottom: 1px solid #F3F4F6; font-weight: 700;">Revisi {{ $document->doc_revision ?? '0' }}</td>
                    </tr>
                    <tr>
                        <td style="font-size: 12px; font-weight: 600; color: #6B7280;">Catatan Revisi</td>
                        <td style="font-size: 12px; color: #DC2626; font-weight: 600; line-height: 1.5;">{{ $notes }}</td>
                    </tr>
                </table>

                <p style="font-size: 13px; line-height: 1.6; color: #4B5563; margin: 0 0 24px 0;">
                    @if(!empty($isCreator))
                        Silakan klik tombol di bawah ini untuk membuka halaman perbaikan dan mengunggah berkas revisi yang baru.
                    @else
                        Silakan klik tombol di bawah ini untuk melihat rincian dokumen.
                    @endif
                </p>
