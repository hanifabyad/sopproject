    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div class="lg:col-span-8 bg-white rounded-lg p-6 shadow-sm border border-sand-200/60 flex flex-col space-y-4">
            ...
            <div class="flex-1 min-h-[750px] bg-canvas rounded-md overflow-hidden border border-sand-200">
                <iframe src="{{ route('admin.BU.document.stream', $document->id) }}#toolbar=0" class="w-full h-full min-h-[750px] border-none"></iframe>
            </div>
        </div>

        <!-- RIGHT COLUMN: METADATA, ATTACHMENTS, & AUDIT TRAIL (COL 4) -->
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm border border-sand-200/60 space-y-3">
                ...
                <div class="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                    @foreach($allAtts as $idx => $att)
                        <a href="{{ route('admin.BU.document.attachment.stream', ['id' => $document->id, 'attId' => $att->id]) }}" target="_blank" class="flex items-center justify-between p-2.5 bg-canvas rounded-md border border-sand-200 hover:bg-[#fff9ed] transition-all text-xs font-semibold text-on-surface">
                    ...
                    <div class="relative flex items-start space-x-3 pl-1">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-1 p-2.5 bg-canvas rounded-md border border-sand-200 text-xs text-on-surface">
                                <p class="italic text-[11px] leading-relaxed">"{{ $log->notes }}"</p>
                            </div>
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm border border-sand-200/60 space-y-3">
                @if($document->status == 'active' && ($pathFinal || $document->file_final))
                    <x-interactive-button text="Unduh Dokumen Sah Final" variant="success" icon="ph ph-download-simple text-base" href="{{ route('admin.BU.document.stream', $document->id) }}" download class="w-full justify-center py-3" />
                @endif
