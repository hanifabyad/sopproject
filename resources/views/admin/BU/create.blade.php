...
<div class="space-y-6 bu-support-scope">
    ...
    <div class="bg-gradient-to-r from-[#1677B8] to-[#00b4d8] text-white rounded-lg p-6 shadow-sm border border-white/10 flex flex-col gap-3">
        ...
        <div class="flex items-center gap-3">
            <x-back-button href="{{ route('admin.BU.show', $unit) }}" variant="light" />
            <span class="text-white/30">|</span>
            <div class="flex items-center gap-2 text-xs text-white/80">
                <a href="{{ route('admin.BU.index') }}" class="hover:text-[#ffe16e] font-medium">Business Unit</a>
                <span>/</span>
                <a href="{{ route('admin.BU.show', $unit) }}" class="hover:text-[#ffe16e] font-medium">{{ $unit }}</a>
                <span>/</span>
                <span class="font-bold text-white">Form Inisiasi SOP Baru</span>
            </div>
        </div>

        <!-- Baris Judul -->
        <div>
            <h2 class="text-xl font-extrabold tracking-tight">Form Inisiasi SOP Baru</h2>
            <p class="text-xs text-white/85 mt-0.5 font-medium">Unggah berkas komponen SOP untuk unit operasional {{ $unit }}</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-12 bg-white rounded-lg p-6 shadow-sm border border-sand-200/60 space-y-5">
                <div class="flex items-center space-x-3 border-b border-sand-200/40 pb-3">
                    <span class="material-symbols-outlined text-2xl text-[#00b4d8]">edit_note</span>
                    <div>
                        <h4 class="font-extrabold text-xs capitalize tracking-wider text-on-surface">Informasi SOP</h4>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div id="bu-additional-approvers" class="hidden fixed inset-0 z-50 bg-charcoal-900/60 backdrop-blur-sm items-center justify-center p-4">
                            ...
                            <div class="flex items-start justify-between gap-4 border-b border-sand-200/60 pb-3 mb-4"><div><p class="text-[10px] font-extrabold capitalize tracking-wider text-gold-500">Pilih tambahan</p><h4 class="text-sm font-extrabold text-on-surface mt-1">Pilih tambahan pengesah</h4></div><button type="button" onclick="closeAdditionalApprovers('bu-additional-approvers')" class="w-8 h-8 rounded-md bg-canvas border border-sand-200 text-on-surface-variant flex items-center justify-center"><span class="material-symbols-outlined text-lg">close</span></button></div>
                            ...
                            @foreach($reviewers as $user)
                                ...
                                @endif
                            @endforeach
                            </div><div class="flex justify-end mt-5"><button type="button" onclick="closeAdditionalApprovers('bu-additional-approvers')" class="px-4 py-2 bg-charcoal-900 text-gold-fixed rounded-md text-[10px] font-extrabold capitalize tracking-wider">Simpan Pilihan</button></div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-12 bg-white rounded-lg p-6 shadow-sm border border-sand-200/60 space-y-6">
                <div class="flex items-center space-x-2 border-b border-sand-200/40 pb-3">
                    ...
                    <h4 class="font-extrabold text-xs capitalize tracking-wider text-on-surface">Upload Berkas PDF Komponen SOP</h4>
                </div>
