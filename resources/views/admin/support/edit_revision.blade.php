...
@section('title', 'Perbaiki Dokumen SOP Support')
@section('header_title', 'Perbaikan Dokumen SOP Departemen Support')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.support.document.detail', $document->id) }}" class="px-3.5 py-1.5 bg-white border border-sand-200 text-charcoal-900 hover:bg-[#fff9ed] rounded-md font-bold text-xs transition-all shadow-sm flex items-center gap-1.5">
    ...
    <div class="bg-white rounded-lg p-6 md:p-8 shadow-sm border border-sand-200/60 space-y-6">
        <div class="border-b border-sand-200/40 pb-4">
            <span class="px-2.5 py-0.5 bg-red-50 text-red-700 font-bold text-[10px] uppercase rounded tracking-wider border border-red-200">Perbaikan Dokumen Support</span>
            <h2 class="text-xl font-extrabold text-on-surface uppercase tracking-tight mt-2">Unggah Perbaikan Dokumen SOP Support</h2>
        ...
        @php
            $authUser = auth()->user();
            $isAdminUser = $authUser->role === 'admin';
            $revisionFormAction = $isAdminUser
                ? route('admin.support.update_revision', $document->id)
                : route('admin.support.creator_update_revision', $document->id);
        @endphp
        <form action="{{ $revisionFormAction }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
