        ...
        <div class="bg-white rounded-2xl shadow-sm p-5 flex items-center justify-between border border-gray-100 md:col-span-2">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <span class="text-[9px] text-gray-400 font-bold uppercase tracking-wider block mt-0.5"><i class="fa-solid fa-briefcase text-[8px] text-gray-300 mr-0.5"></i> Departemen: {{ $document->department }}</span>
                </div>
            </div>
            
            <a href="{{ route('admin.support.show', $document->department) }}" class="text-[9px] font-bold text-gray-400 uppercase tracking-wider bg-slate-50 border border-slate-100 px-3 py-2 rounded-xl hover:bg-slate-100 transition-colors">
                <i class="fa-solid fa-arrow-left mr-1"></i> List {{ $document->department }}
            </a>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="reviewer_id" class="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl text-xs font-bold text-[#1e293b] focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-all cursor-pointer">
                            @foreach($reviewers as $rev)
                                <option value="{{ $rev->id }}" {{ $document->reviewer_id == $rev->id ? 'selected' : '' }}>
            ...
            <div class="bg-white rounded-2xl shadow-md p-4 border border-gray-100 flex flex-col gap-3">
                ...
                <form action="{{ route('admin.support.document.delete', $document->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus dokumen ini secara permanen dari server database?')">
                    @csrf @method('DELETE')
