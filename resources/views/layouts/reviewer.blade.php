    ...
    <style>
        ...
        @media (min-width: 1024px) {
            ...
            aside.w-64 .sidebar-item-active {
                ...
                margin-right: -1.5rem !important; /* extends past right padding */
...
<body class="bg-canvas text-on-surface h-full font-sans antialiased lg:overflow-hidden overflow-y-auto selection:bg-gold-fixed selection:text-charcoal-900">
        ...
        <aside id="sidebar" class="w-64 bg-gradient-to-b from-[#1677B8] to-[#00b4d8] text-white flex-shrink-0 flex flex-col z-40 fixed inset-y-0 left-0 transform -translate-x-full lg:translate-x-0 transition-all duration-200 ease-in-out rounded-r-3xl">
            ...
            <div onclick="handleBrandClick()" class="sidebar-brand-wrapper p-5 flex items-center justify-between border-b border-white/10 text-center relative group transition-all duration-200">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="text-left sidebar-logo-text">
                        ...
                        <span class="text-[9px] font-bold text-[#ffe16e] uppercase tracking-[0.1em] mt-0.5 block">Reviewer Desk</span>
                    </div>
                </div>
            ...
            <nav class="mt-3 flex-1 px-3 space-y-1.5 overflow-y-auto overflow-x-hidden custom-scrollbar">
                ...
                <a href="{{ route('reviewer.dashboard') }}" 
                   class="sidebar-item flex items-center px-3.5 py-2.5 rounded-lg font-semibold text-xs tracking-wide transition-all duration-150 group {{ request()->routeIs('reviewer.dashboard') ? 'bg-canvas text-[#1677B8] font-extrabold sidebar-item-active' : 'text-white/85 hover:bg-white/10 hover:text-white' }}">
                ...
                <a href="{{ route('library.index') }}" 
                   class="sidebar-item flex items-center px-3.5 py-2.5 rounded-lg font-semibold text-xs tracking-wide transition-all duration-150 group {{ request()->routeIs('library.*') ? 'bg-canvas text-[#1677B8] font-extrabold sidebar-item-active' : 'text-white/85 hover:bg-white/10 hover:text-white' }}">
                    <i class="ph ph-books text-lg mr-2.5 {{ request()->routeIs('library.*') ? 'text-[#1677B8]' : 'text-white/90' }}"></i>
            ...
            <div class="p-3 mt-auto space-y-3 border-none outline-none">
                ...
                <div class="sidebar-profile-card flex items-center space-x-3 p-2.5 rounded-sm bg-white border-0 text-on-surface transition-all duration-200">
                    ...
                    <div class="min-w-0 flex-1 sidebar-profile-text text-left">
                        <p class="text-xs font-extrabold text-on-surface truncate leading-tight">{{ Auth::user()->name ?? Auth::user()->username }}</p>
                        <p class="text-[9px] text-gold-dim font-bold tracking-wider mt-0.5 truncate">{{ Auth::user()->email ?? 'reviewer@eqms.com' }}</p>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Jika navigasi dibatalkan atau halaman dipulihkan, jangan biarkan
            // container tetap transparan.
