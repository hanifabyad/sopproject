        ...
        <aside class="w-64 bg-[#1e293b] text-white flex-shrink-0 flex flex-col shadow-xl z-20">
            ...
            <div class="p-6 flex flex-col items-center space-y-3 border-b border-white/5 w-full text-center">
                <div class="bg-white p-3 rounded-[2rem] shadow-lg shadow-blue-500/5 flex items-center justify-center w-24 h-24 transition-transform duration-300 hover:scale-105">
                    <img src="{{ asset('img/logo pkm 2.png') }}" class="w-full h-full object-contain" alt="Logo PKM">
                </div>
            ...
            <nav class="mt-6 flex-1 px-4 space-y-1.5 overflow-y-auto custom-scrollbar">
                <a href="{{ route('reviewer.dashboard') }}" 
                   class="flex items-center px-4 py-3 rounded-xl transition-all duration-200 transform hover:translate-x-1 group {{ request()->routeIs('reviewer.dashboard') ? 'bg-blue-600 text-white shadow-md shadow-blue-600/10' : 'hover:bg-white/5 text-gray-400 hover:text-white' }}">
                    <div class="w-5 text-center text-sm">
                        <i class="fa-solid fa-hourglass-half"></i>
                    </div>
                    <span class="ml-3.5 font-bold text-xs uppercase tracking-wider">Antrian Review</span>
                </a>

                <a href="{{ route('reviewer.history') }}" 
                   class="flex items-center px-4 py-3 rounded-xl transition-all duration-200 transform hover:translate-x-1 group {{ request()->routeIs('reviewer.history') ? 'bg-blue-600 text-white shadow-md shadow-blue-600/10' : 'hover:bg-white/5 text-gray-400 hover:text-white' }}">
                    <div class="w-5 text-center text-sm">
                        <i class="fa-solid fa-clock-rotate-left"></i>
                    </div>
                    <span class="ml-3.5 font-bold text-xs uppercase tracking-wider">Riwayat Selesai</span>
                </a>
                ...
                <a href="{{ route('library.index') }}" 
                   class="flex items-center px-4 py-3 rounded-xl transition-all duration-200 transform hover:translate-x-1 group {{ request()->routeIs('library.*') ? 'bg-blue-600 text-white shadow-md shadow-blue-600/10' : 'hover:bg-white/5 text-gray-400 hover:text-white' }}">
                    ...
                    <span class="ml-3.5 font-bold text-xs uppercase tracking-wider">E-Library</span>
                </a>
            </nav>
            ...
            <div class="p-4 border-t border-white/5">
                <form action="{{ route('logout') }}" method="POST">
                    ...
                    <button type="submit" class="flex items-center w-full px-4 py-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-all duration-200 font-bold group">
                        ...
                        <span class="ml-3.5 text-[11px] uppercase tracking-widest whitespace-nowrap">Keluar Sistem</span>
                    </button>
                </form>
            </div>
        </aside>
        ...
        <div class="flex-1 flex flex-col min-w-0 bg-slate-50 relative">
            <header class="h-16 border-b border-gray-200 flex items-center justify-between px-8 bg-white z-10 shadow-sm">
                <h2 class="text-sm font-black text-[#1e293b] uppercase tracking-tight flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full bg-blue-600 block"></span> e-QMS Document Review Portal
                </h2>
                
                <div class="flex items-center space-x-4">
                    <div class="text-right hidden sm:block">
                        <p class="text-[11px] font-black text-[#1e293b] uppercase leading-none">{{ Auth::user()->username }}</p>
                        <p class="text-[9px] text-blue-600 font-bold tracking-wider mt-1 uppercase">Kepala Dept. {{ Auth::user()->department }}</p>
                    </div>
                    <div class="w-9 h-9 rounded-xl bg-blue-900 flex items-center justify-center text-white font-extrabold shadow-md shadow-blue-900/10 uppercase tracking-tighter text-sm border-2 border-white">
                        {{ substr(Auth::user()->username ?? 'P', 0, 1) }}
                    </div>
                </div>
            </header>
            ...
            <main class="flex-1 overflow-y-auto p-8 custom-scrollbar">
                @if(session('success'))
                    <div class="mb-6 p-4 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-700 font-bold text-xs uppercase tracking-wide rounded-r-xl shadow-sm flex items-center gap-2">
